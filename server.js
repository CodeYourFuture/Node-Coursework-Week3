...
app.post("/bookings", (request, response) => {
  //validation for room-id - check the data before creating the new Booking
  //to be refactored // is roomid reserverd
  //1. check if room is taken and check if room exist against fixed number (e.g: < 10)
  //2. if yes, check the date coming from the user agaisnt database
  //3. if not, send bad request
  let keys = [
    "title",
    "firstName",
    "surname",
...
app.get("/bookings/:id", (request, response) => {
  ...
  if (foundBooking) {
    ...
    response.status(400).json("Sorry, booking not found");
  }
});
