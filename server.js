...
//     const bookingId = parseInt(req.params.id);
//     bookings.find(e => {
//         e.id === bookingId ? res.status(200).send(e) : res.status(404).send(`Incorrect id given`);
//     });
// });
...
// app.delete('/bookings/delete/:id', (req, res)=>{
//     const deleteId = parseInt(req.params.id);
//     let delBookings = bookings.filter(element =>{
//         bookings = delBookings;
//         element.id !== deleteId ? res.status(200).send(bookings) : res.status(404).send(`No booking with this id`);
//     });
// }); 
