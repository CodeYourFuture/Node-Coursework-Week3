...
app.post("/bookings", (req, res) => {
  ...
  if (!title || !surname || !email || !checkInDate || !checkOutDate) {
    res.status(400).send({
      message: " ERROR All fields have not been completed please try again",
    });
  }
  let newBookingTable = {
    id: 0,
...
app.get("/bookings/:id", function (request, response) {
  let bookingIndex = bookings.findIndex(
    (newBookingTable) => newBookingTable.id == request.params.id
  );
...
app.delete("/bookings/:id", function (request, response) {
  let bookingIndex = bookings.findIndex(
    (newBookingTable) => newBookingTable.id == req.params.id
  );
